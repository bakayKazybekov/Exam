...
const scheme = yup.object({
    ...
    confirmPassword: yup.string().required('Поле обязательно').min(8, 'Не менее 8-ми')
...
const Registration = ({ setIsRegister, onSubmit }) => {
    const {
        register,
        handleSubmit,
        formState: { errors }
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className={s.wrapper}>
            ...
            <div>Есть аккаунт? <span className={s.wrapper_pass} onClick={() => setIsRegister(false)}>Войдите</span></div>
